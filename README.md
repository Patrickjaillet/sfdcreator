...
> **Status: early development (Phase 5 — Timeline & Synchronization Engine).** The docked timeline widget is now backed by a real engine: keyframed parameter automation (cube rotation speed), a two-segment master timeline with a crossfade transition between two camera paths, BPM-grid beat snapping, play/pause/scrub, and undo/redo (Ctrl+Z/Ctrl+Y) on keyframe edits. A screenshot will be added here once the UI is visually validated.

## Tech stack

- **C# 12** on **.NET 8** (Windows 10/11 only)
- A framework-agnostic **timeline & synchronization engine** in `SFDCreator.Core` — see [src/SFDCreator.Core/Timeline](src/SFDCreator.Core/Timeline) — keyframe interpolation (linear/bezier/stepped), parameter-curve automation, a master timeline with segment sequencing and cut/crossfade transitions, play/pause/seek playback control, a BPM beat grid, and a command-pattern undo/redo stack. No embedded scripting language (that's Phase 14's explicit scope) and no audio-driven beat detection yet (that's Phase 8's — the BPM grid takes a manually-set tempo for now).
- A hand-written native **Win32** window host (real HWND, own WndProc/message loop) — see [src/SFDCreator.Win32](src/SFDCreator.Win32) — with native menu bar, docking-region plumbing (five regions: Top/Left/Right/Bottom/Center), Open/Save dialogs, drag-and-drop, DPI/multi-monitor awareness, and an input bridge implementing Silk.NET's `IInputContext`/`IKeyboard`/`IMouse`
...
- **SkiaSharp** (CPU raster, blitted via GDI `SetDIBitsToDevice`) for the docked editor UI — see [src/SFDCreator.UI](src/SFDCreator.UI) — a small immediate-mode widget kit (buttons/sliders/checkboxes), a property inspector live-bound to the renderer's post-processing parameters, a node graph editor, and a timeline widget now wired to the real Core timeline engine, all themeable (Dark/Light)
...
| Project | Purpose |
| --- | --- |
| `SFDCreator.Core` | Plugin/module architecture, shared services, settings abstraction, timeline & synchronization engine |
...
| `SFDCreator.App` | Composition root — wires the native Win32 host, docked UI panels, timeline engine, and the OpenGL render loop into a running application |
